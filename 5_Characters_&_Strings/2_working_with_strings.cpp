/*
C++ Strings, have dinamic size, works with imput and output streams, has a lot of useful functions,
can use operators with them (+, !=...), can convert to C-Style

! #include <string>
Strings are always initialized to empty

s1 += " James";     / concat smt to the string
cin >> s1;      / works until space is found
===========================================================================================
! getline(cin, s1)    / imput of full user text
getline(cin, s1, 'x')      / limits the imput of variable till condition 'x' is found

string.clear();
string.lenght();    / returns integer of the size

string.at(x);       / OOP
== != > >= < <=     / compare

! string.substr(start_index, length)    // subtring of caracters defined
! string.erase(start_index, length);     // erase those characters

? string.find("Example");   // returns the position it is located ===== string::npos  (if not found)
...
int main()
{
     ...
     cout << "\nComparison"
          << "\n------------------------------------------" << endl;
     cout << boolalpha;                                        // get true or false instead of 1, 0
